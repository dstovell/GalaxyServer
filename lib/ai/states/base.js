...
AIBaseState.prototype.step = function(async, brain, options, cb){
	...
	self._stepState_setGoalValues(async, brain, options, function(err) {
		if (err) {
			return cb(err);
		}

		self._stepState_act(async, brain, options, function(err) {
			if (err) {
				return cb(err);
			}

			self._stepState_stateChange(async, brain, options, function(err, newState) {
				if (err) {
					return cb(err);
				}
