...
AIBaseState.prototype._stepState_stateChange = function(async, brain, options, cb){
	...
	var entities = {"self":brain};
	
	//console.log(self.name + " _stepState_stateChange totalConditionals:", totalConditionals);

	var newState = null;

	for (var i=0; i<totalConditionals.length; i++) {
		var conditional = totalConditionals[i];
		
		var evalResult = self.ai_conditional_evaluator.evaluateConditional(conditional, entities, options);
