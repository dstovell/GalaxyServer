...
AIBaseEntity.prototype._setupChecks = function(){
	...
	self.checks.ageEquals =  {
		params: {"age":"number"},
		ret: "bool",
		func: function(params){
			return (self.stepCount === params.age);
		}
	};
...
AIBaseEntity.prototype.runCheck = function(name, params){
	...
	var check =  self.checks[name];
	check.params = check.params || {};
	for (var k in check.params) {
		if (check.params[k] === "number") {
			params[k] = parseFloat(params[k]) || 0;
		}
	}

	return self.checks[name].func.call(self, params);
};
...
AIBaseEntity.prototype.runAction = function(name, params, cb){
	...
	return self.actions[name].func.call(self, params, cb);
};

exports.createStub = function(options) { return new AIBaseEntity("base", options); };
