...
AIBrainEntity.prototype._setupActions = function(){
	...
	self.actions.selfDestruct =  {
		...
		func: function(params, cb){
			var eventString = "[AIBrainEntity] " + self.name + " selfDestruct step:" + this.stepCount;
			console.log(eventString);
			return cb(null, [eventString]);
		}
	};
};

AIBrainEntity.prototype.step = function(async, options, cb){
	var self = this;

	self.stepCount++;

	self.currentState.step(async, self, options, function(err, result) {
