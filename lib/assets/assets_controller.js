...
function AssetsController(options) {
	...
	self.users_controller = require('../../lib/users/users_controller').create(options);
...
AssetsController.prototype.addAsset = function(assetKey, uid, starId, options, cb){
	...
	self.users_controller.getUser(uid, function(err, user) {
		if (err || !user) {
			return cb(err || "noUser", user);
		}
...
AssetsController.prototype.getLocation = function(asset, starTable, options) {
	var self = this;
	options = options || {};
	...
	var isBetweenTheseTimes = function(a, d) {
		return ( ((a == null) || (a < now)) && ((d == null) || (d > now)) );
	};

	var course = asset.course || [];
	for (var i=0; i<course.length; i++) {
		var c = course[i];
		if ( isBetweenTheseTimes(c.ats, c.dts) ) {
			if (c.starId) {
				if (c.localCourse) {
					for (var j=0; j<c.localCourse.length; j++) {
						var lc = c.localCourse[j];
						if ( isBetweenTheseTimes(lc.ats, lc.dts) ) {
							//In orbit, docked or in a raw position
						}
						else if (j > 0) {
							var lastLC = course[j-1];
							if ( isBetweenTheseTimes(lastLC.dts, lc.ats) ) {
								loc.state = "sublight";
								//interpolate local locations/raw solar positions for current solar position
							}
						}
					}
				}
				else {
					loc.state = "solarspace";
					loc.pos = c.pos;
					return loc;
				}
			}
			...
			var lastC = course[i-1];
			if ( isBetweenTheseTimes(lastC.dts, c.ats) ) {
				loc.state = "warping";
				//interpolate galactic locations/raw intesteller positions for current intesteller position
				var star1 = starTable[lastC.starId];
				var star2 = starTable[c.starId];
			}
		}
	}
...
AssetsController.prototype.buildAssetCourse = function(asset, uid, destStarId, options){
	var self = this;
	options = options || {};
