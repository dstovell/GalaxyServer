...
function getDistance(pos1, pos2) {
	var dist = Math.sqrt( getDistanceSquared(pos1, pos2) );
	return dist;
};
...
function updateDragCamera(pgame, obj) {
	...
	if (obj._wasTouching1 && obj._wasTouching2) {

	}
	else if (obj._wasTouching1) {
   		pgame.world.pivot.x += obj._dX1/(pgame.world.scale.x*1);
   		pgame.world.pivot.y += obj._dY1/(pgame.world.scale.y*1);
   	}
}
...
function zoom(pgame, amount) {
	worldScale += amount * worldScale;
	worldScale = Math.max(worldScale, 1.0);
	worldScale = Math.min(worldScale, 40.0);
	pgame.world.scale.set(worldScale);
}
...
function updateDrag(pgame, obj) {
	...
	var touch1 = null;
	var touch2 = null;
    if (pgame.input.mousePointer && pgame.input.mousePointer.isDown) {
    	touch1 = pgame.input.mousePointer;
    }
    
    if (pgame.input.pointer1 && pgame.input.pointer1.isDown) {
    	touch1 = pgame.input.pointer1;
    }

    if (pgame.input.pointer2 && pgame.input.pointer2.isDown) {
    	touch2 = pgame.input.pointer2;
    }

    if (touch1 && touch2) {
    	var dist = getDistance({x:touch1.clientX, y:touch1.clientY}, {x:touch2.clientX, y:touch2.clientY});

    	if (!obj._wasTouching1 || !obj._wasTouching2) {
    		obj._dis = dist;
    		obj._wasTouching1 = true;
    		obj._wasTouching2 = true;
    	}
    	else if (dist > obj._dis) {
    		zoom(pgame, 0.1);
    	}
    	else if (dist < obj._dis) {
    		zoom(pgame, -0.1);
    	...
    	if (!obj._wasTouching1) {
    		obj._wasTouching1 = true;
    		obj._dX1 = 0;
    		obj._dY1 = 0;
    	}
    	else {
    		obj._dX1 = obj._lastTouchX1 - touch1.clientX;
    		obj._dY1 = obj._lastTouchY1 - touch1.clientY;
    	}

    	obj._lastTouchX1 = touch1.clientX;
    	obj._lastTouchY1 = touch1.clientY;
    	//return {x:obj._lastTouchX, y:obj._lastTouchY, dX:obj._dX, dY:obj._dY};
    }
    else {
    	obj._wasTouching1 = false;
    	obj._wasTouching2 = false;
    	obj._dX1 = 0;
    	obj._dY1 = 0;
    	obj._dis = 0;
    }
